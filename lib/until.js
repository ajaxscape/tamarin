...
const until = (world, until) => {
  ...
  _.extend(until, {
    ...
    notFoundInPage: (selector) => new webDriver.Condition(`for $("${selector}") to not be found in page`, co(function * (selector) {
      const driver = yield world.getDriver()
      return yield driver.findElement(By.css(selector))
        .then(() => false)
        .catch(() => true)
    })),
    browserReady: () => new webDriver.Condition('for url to not equal data ', co(function * () {
      const driver = yield world.getDriver()
      const url = yield driver.getCurrentUrl()
      return url !== 'data:,'
    })),
    cookieExists: (cookieName) => new webDriver.Condition(`for cookie "${cookieName}" to exist`, co(function * (cookieName) {
      const driver = yield world.getDriver()
