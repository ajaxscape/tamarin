...
let World = (() => {
  ...
  class World {
    ...
    click (id, retries, timeout) {
      let click = (retries) => find(id, this, timeout)
        .then((el) => whenVisible(el, timeout))
        .then((el) => el.click())
        .catch((err) => {
          if (retries) {
    ...
    sendKeys (id, text, retries, timeout) {
      let sendKeys = (retries) => find(id, this, timeout)
        .then((el) => whenVisible(el, timeout))
    ...
    visit (page, params) {
      return getRoute(page)
        .then((route) => {
          ...
          let paths = _.cloneDeep(_.isArray(route.path) ? route.path : route.path ? [route.path] : [])
          let routePath
