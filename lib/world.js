...
var notFind = co(function* (id, timeout) {
    yield driver.wait(until.configuredInPage(id), timeout || defaultTimeout);
    var selector = world.currentPage.config[_.camelCase(id)];
...
World.prototype.notFind = notFind;
...
World.prototype.getHtml = (id) => find(id)
  .then(el => el.getAttribute('outerHTML'));

World.prototype.select = (id, query) => world.getHtml(id)
  .then(html => {
...
function matchedRoute(routes, val) {
    ...
    routes.some(function (route) {
        ...
        return (_.isArray(route.path) ? route.path : [route.path]).some(function(path){
            var parts = path.split('/').map(function (part, index) {
            if (part[0] === ':') {
    ...
    return new Promise(function (resolve) {
        getRoute(page)
            .then((route) => {
                ...
                var path = config.host + (_.isArray(route.path) ? route.path[0] : route.path);
                if (params) {
