...
let getSelector = (config, id) => _.reduce(id.split(':'), (prev, next) => {
  if (_.isArray(prev)) {
    if (prev.length === 2) {
      let selector = prev[0];
      if (_.isArray(prev[1])) {
        selector += ' ' + prev[1][0];
        return [selector, prev[1][1][_.camelCase(next)]]
      } else {
        return [selector, prev[1][_.camelCase(next)]]
      }
    }
    return [prev[0][_.camelCase(next)]];
  } else {
    return [prev[_.camelCase(next)]];
  }
}, config);
...
let whenPageIs = co(function* (page, world, timeout) {
  yield driver.wait(until.pageMatches(page, world), timeout || defaultTimeout);
  return Promise.resolve();
});
...
let find = co(function* (id, world, timeout) {
  let selector = 'body';
  if (id) {
    yield driver.wait(until.configuredInPage(id, world), timeout || defaultTimeout);
