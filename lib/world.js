...
let whenBrowserReady = co(function* (timeout) {
        ...
        getRoute(page)
          .then((route) => {
            ...
            if (params) {
              ...
              let parts = route.path.split('/').map(function (part) {
                if (part[0] === ':') {
...
let matchedRoute = function (routes, val) {
  let matched = false;
...
let getRoute = (val, prop) => new Promise((resolve, reject) => loadRoutes()
  .then((routes) => {
    ...
    let route = _(routes).find(function (route) {
...
let getPageObject = () => new Promise((resolve, reject) => whenBrowserReady()
  .then((url) => getRoute(url.substr(config.host.length), 'path')
    .then(function (route) {
      if (!route) {
        throw new Error(`Route is not defined for "${url}"`);
      }
      let pageObject = buildPageObject.call(world, route.pageObject);
      if (pageObject) {
...
let buildPageObject = (pageConfig) => {
