...
var World = function World() {
    ...
    until.titleIs = function (expectedTitle) {
        return new until.Condition('for ' + expectedTitle + ' to match page title ', co(function* () {
...
World.prototype.whenVisible = co(function* (el) {
    yield driver.wait(until.elementIsEnabled(el), defaultTimeout);
    yield driver.wait(until.elementIsVisible(el), defaultTimeout);
    return Promise.resolve(el);
});

World.prototype.whenMatches = co(function* (el, text) {
    yield driver.wait(until.elementTextIs(el, text), defaultTimeout);
    return Promise.resolve(el);
});

World.prototype.whenTitleIs = co(function* (title, timeout) {
    yield driver.wait(until.titleIs(title), timeout || defaultTimeout);
    return Promise.resolve();
});

World.prototype.whenPageIs = co(function* (page, timeout) {
    yield driver.wait(until.pageMatches(page), timeout || defaultTimeout);
    return Promise.resolve();
});

/* Find the id in the config and use the selector to find the element and wait until it is visible */
...
World.prototype.whenTextMatches = function (id, text) {
    return this.find(id)
        .then(el => this.whenVisible(el))
        .then(el => this.whenMatches(el, text))
...
module.exports.setRoutes = myRoutes => routes = myRoutes;
