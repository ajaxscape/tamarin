...
let getSelector = (config, id) => _.reduce(id.split(':'), (prev, next) => prev[_.camelCase(next)], config);
...
_.extend(until, {
  ...
  configuredInPage: (id, world) => new until.Condition(`for "${id}" to be configured in page`, co(function* () {
    let page = yield getPageObject(world);
    return _.isString(getSelector(page.config, id));
  })),
...
let find = co(function* (id, world, timeout) {
  ...
  if (id) {
    ...
    selector = getSelector(world.currentPage.config, id);
  }
