...
World.prototype.expectPageToEventuallyBe = function (page, next) {
    var route = this.getRoute(page);
    if (!route) {
        throw new Error('Route is not defined for "' + page + '" page');
    }
    driver.wait(() => new until.Condition('matching page', () => this.currentPage.route === route), 10000).then(() => next());
};

World.prototype.findSelector = function findSelector(id) {
    var key = _.camelCase(id);
    return new Promise((resolve) => driver.wait(() => new until.Condition('configured in page', () => this.currentPage.config.hasOwnProperty(key)), defaultTimeout)
        .then(() => {
...
World.prototype.find = function (id) {
    return new Promise((resolve) => this.findSelector(id)
        .then((selector) => {
            return driver.wait(() => new until.Condition('found in page', () => driver.findElement(webdriver.By.css(selector))), defaultTimeout)
