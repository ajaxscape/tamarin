...
_.extend(until, {
    ...
    foundInPage: (selector) => new until.Condition('for $(' + selector + ') to be found in page ', function () {
...
World.prototype.sendKeys = (id, text) => find(id)
    ...
    .then(el => {
        el.sendKeys(text);
        return world.driver.executeScript((browserEl) => {$(browserEl).change()}, el)
    });
...
var getPageObject = () => new Promise(function (resolve, reject) {
    world.driver.getCurrentUrl().then(function (url) {
        return getRoute(url.substr(config.host.length), 'path')
            .then(function (route) {
