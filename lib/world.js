...
World.prototype.find = function (id) {
    return driver.wait(() => new until.Condition('configured in page', () => !!this.currentPage.config[id]), defaultTimeout)
        .then(() => driver.wait(until.elementLocated(webdriver.By.css(this.currentPage.config[id]))));
};

World.prototype.untilVisible = function (id) {
    return this.find(id)
        .then((el) => this.world.driver.wait(until.elementIsVisible(el)));
};
