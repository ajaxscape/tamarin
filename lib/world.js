...
const World = (() => {
  ...
  class World {
    ...
    getDriver (driver) {
      if (!_driver.get(this)) {
        if (driver) {
          ...
          return Promise.reject(new Error('Expected a driver to be passed in the world constructor!'))
        }
      }
    ...
    waitFor (selector, retries, timeout) {
      return waitFor(this, selector,
        (el) => Promise.resolve(el), _.isUndefined(retries) ? 5 : retries, timeout)
    }
